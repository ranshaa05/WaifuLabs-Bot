...
class Reaction(nextcord.ui.View):
    ...
    def __init__(self, site_navigator):
        super().__init__()
        self.buttons = []
        self.site_navigator = site_navigator
        label_list = []
        color_list = []
        emoji_label_list = ["❌", "⬅", "➡", "🤷‍♂️", "🔄"]
    ...
    async def click_by_label(self, label):  # label is the string that is shown on the button
        if label.isdecimal():
            await self.site_navigator.click_by_index(int(label))
            Reaction.stage = Reaction.stage + 1
            self.stop()

        elif label == "❌":
            await self.site_navigator.exit()
            self.stop()
        elif label == "⬅":
            await self.site_navigator.undo()
            Reaction.stage = Reaction.stage - 1
            self.stop()
        elif label == "➡":
            await self.site_navigator.keep()
            Reaction.stage = Reaction.stage + 1
            self.stop()
        elif label == "🤷‍♂️":
            await self.site_navigator.rand()
            Reaction.stage = Reaction.stage + 1
            self.stop()
        elif label == "🔄":   #refresh
            await self.site_navigator.refresh()
            self.stop()
        else:
